...
def test_tree_of_nuance():
    with patch.dict(sys.modules, get_mock_modules()):
        # Import inside context to ensure mocks are picked up
        ...
        try:
            ...
            db.commit()
            logging.info(f"Step 1: Injected {len(fragment_ids)} fragments linked to '{target_entity}'.")

            cursor.execute("SELECT id, content_full, status, importance_score FROM memories")
            logging.info(f"Database Memories: {cursor.fetchall()}")
            cursor.execute("SELECT * FROM memory_associations")
            logging.info(f"Database Associations: {cursor.fetchall()}")
            ...
            logging.info(f"Step 2: Observation Anchor created (ID: {anchor_id}).")
            logging.info(f"Summary Content: {anchor_text[:100]}...")
            ...
            logging.info(f"Step 3: Found {len(children)} CHILD_OF links to Anchor.")
            ...
            logging.info("Step 4: Retriever successfully found Anchor.")
            assert "[Nuanced Details available via recall_reverie" in found_anchor["content"]
            logging.info("Step 5: Retriever correctly signaled Child IDs in context.")

            # 7. Verification: Tool Functionality (recall_reverie)
            child_id = fragment_ids[0]
            # Test authorized recall
            logging.info(f"Step 6: Testing safe recall of Child ID {child_id}...")
            ...
            logging.info("Step 7: Testing unauthorized recall (Expect Security Denial)...")
            resp = provider_inst.handle_tool_call("recall_reverie", {"memory_id": child_id})
            assert "Access Denied" in resp
            logging.info("Step 8: Security validation SUCCESSFUL.")
